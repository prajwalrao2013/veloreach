            ...
            if (source === 'G-Maps') {
                ...
                for (const jid in participantGroups) {
                    ...
                    for (const participant of group.participants) {
                        ...
                        const baseJid = `${contactId}@s.whatsapp.net`;
                        const cachedContact = phoneContacts[participant.id] || phoneContacts[baseJid];
                        const name = cachedContact?.name || cachedContact?.notify || cachedContact?.verifiedName || contactId;
        ...
        try {
            if (!aiChatSession) {
                ...
                aiChatSession = model.startChat({
                    history: [
                        { role: "user", parts: [{ text: "You are VeloReach AI, a high-level enterprise WhatsApp marketing and automation expert. Provide concise, strategic responses." }] },
                        { role: "model", parts: [{ text: "Acknowledged. I am initialized and ready to assist with VeloReach strategy." }] }
                    ]
                });
            }
    ...
    socket.on('launch_campaign', async (data) => {
        ...
        for (let i = 0; i < leads.length; i++) {
            ...
            try {
                ...
                console.error(`[Campaign] Error dispatching to ${target.contact}:`, err.message);
